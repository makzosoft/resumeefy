/**
 * Resumeefy AI provider: Google Gemini.
 *
 * Resumeefy does not use OpenAI. All AI features (resume generation, quality
 * scoring, job matching, interview questions/feedback, desktop simulation,
 * course and blog generation) go through Gemini's OpenAI-compatible chat
 * completions endpoint below. Configure GEMINI_API_KEYS and GEMINI_MODEL
 * (see .env.example) — there is no OPENAI_API_KEY anywhere in this app.
 */
import { RESUMEEFY_CV_SYSTEM_PROMPT } from "./resume-rules";

// NOTE: the "/openai/" segment here is part of Google's real, published URL for
// its OpenAI-compatible endpoint — it is not a leftover OpenAI integration.
...
async function responsesJson<T>(
  ...
  while (true) {
    ...
    try {
      const result = await callGemini<T>(RAW_KEYS[idx], input, schema, schemaName, instructions);
